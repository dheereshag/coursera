...
__all__ = ["MODAL_BTN", "NEXT_BTN", "SUB_SEL", "poll_and_click_next", "submit_quiz"]

logger = logging.getLogger(__name__)
SUB_SEL = 'button[data-testid="submit-button"], button:has-text("Submit"):not([data-testid*="dialog"])'
MODAL_BTN = 'button[data-testid="dialog-submit-button"], [role="alertdialog"] button:has-text("Submit"), [role="dialog"] button:has-text("Submit")'
...
def _confirm_modal(page: Page) -> None:
    if (modal := page.locator(MODAL_BTN).first).is_visible(timeout=3000):
        logger.info("Confirming submission in modal dialog...")
        _click_btn(modal)
        with suppress(Error):
